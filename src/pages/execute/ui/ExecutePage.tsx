import { usePomodoroStore } from '@/entities/pomodoro';
import { useLeverageStore } from '@/entities/leverage';
import { Badge } from '@/shared/core/badge';
import {
  Card,
  GuideMessage,
  PhaseNavigation,
  SectionLabel,
  DimmedSection,
  GoalSection,
  InspectionSection,
  InsightCheckList,
} from '@/shared/primitives';
import { Block } from '@/features/block-editor';
import MemoSection from './MemoSection';

export const ExecutePhase = () => {
  const { pomodoro, setTasks, setMemos, setPhase } = usePomodoroStore();
  const { getByTiming } = useLeverageStore();

  const duringInsights = getByTiming('during');

  // depth 0 완료 카운트 (배지용)
  const completedDepth0Count = pomodoro.tasks.filter(
    (t) => t.depth === 0 && t.status === 'deleted'
  ).length;
  const totalDepth0Count = pomodoro.tasks.filter((t) => t.depth === 0).length;

  const handleReflect = () => {
    setPhase('reflect');
  };

  return (
    <div className="space-y-6">
      {/* 헤더 */}
      <div className="flex items-center justify-between">
        <GuideMessage>과제를 완료하면 체크하세요.</GuideMessage>
        <Badge variant="secondary">
          {completedDepth0Count} / {totalDepth0Count} 완료
        </Badge>
      </div>

      {/* 메인 카드 */}
      <Card>
        <Card.Content>
          {/* 목표 */}
          <GoalSection goal={pomodoro.goal} readonly dimmed />

          {/* 과제 */}
          <div className="space-y-2">
            <SectionLabel>과제</SectionLabel>
            <Block.Editor
              blocks={pomodoro.tasks}
              onChange={setTasks}
              showOrder
              droppableId="tasks"
            />
          </div>

          {/* 점검 */}
          <DimmedSection>
            <InspectionSection
              probability={pomodoro.probability}
              tasks={pomodoro.tasks}
            />
          </DimmedSection>
        </Card.Content>
      </Card>

      {/* 작업 중 Insight */}
      <InsightCheckList
        insights={duringInsights}
        label="작업 중 기억하세요"
        variant="badge"
      />

      {/* 작업 중 메모 */}
      <MemoSection memos={pomodoro.memos} onChange={setMemos} />

      {/* 이전/다음 버튼 */}
      <PhaseNavigation
        onBack={() => setPhase('plan')}
        onNext={handleReflect}
      />
    </div>
  );
};
